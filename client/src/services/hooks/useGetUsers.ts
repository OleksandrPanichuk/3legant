...
import { FindAllUsersResponse, UserApi } from "@/services"
import {  useCallback, useEffect, useMemo, useState } from "react"

const TAKE_USERS = 10


type FetchMoreState = 'can-fetch-more' | 'can-fetch-one-more' | 'cannot-fetch-more'


type UseGetUsersResult = UseQueryResult<FindAllUsersResponse, Error> & {
    page:number 
    maxPages: number
    fetchNextPage:() => void
    fetchPrevPage: () => void
...
export const useGetUsers = (searchValue?:string, take: number = TAKE_USERS): UseGetUsersResult => {
    ...
    const [canFetchMore, setCanFetchMore] = useState<FetchMoreState>('can-fetch-more')
    ...
    const fetchNextPage = useCallback(() => {setPage(prev => prev +1)},[])
        ...
        if(!isPlaceholderData) {
            queryClient.prefetchQuery({
                ...
                queryFn: async () =>  {
                    ...
                    const response = await UserApi.findAll({searchValue, take, skip: (page + 1) * take})
    ...
    return {...queryState, maxPages, isPlaceholderData, page, fetchNextPage, fetchPrevPage, canFetchMore}
}